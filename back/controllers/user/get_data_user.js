...
async function getUser(req, res, next) {
  ...
  try {
    ...
    const [result] = await connection.query(
      ...
      surname,
      email,
      birthdate,role,avatar FROM users WHERE user_id=?`,
      [id]
    );
    ...
    if (userData.user_id === req.auth.id || req.auth.role === 'admin') {
      ...
      payload.date_account_creation = formatDateToDB(
        userData.date_account_creation
      );
      payload.username = userData.username;
      ...
      payload.newBirthdate = userData.birthdate;
    } else {
